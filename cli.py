...
class ArgumentParser:
        ...
        def _setup_arguments(self) -> None:
                self.parser.add_argument('-l', '--platform', required=True, 
                                help='Streaming platform name (e.g., douyin, bilibili)')
                self.parser.add_argument('-i', '--id', required=True, 
...
class OutputFormatter:
        @staticmethod
        def format_response(json_data: str, platform: str, room_id: str) -> str:
...
class ErrorHandler:
        ...
        def handle(exception: Exception):
                ...
                print("\nPlease ensure:")
                print("1. You have installed the streamget library (pip install streamget)")
                print("2. Platform name and ID are correct")
                print("3. Proxy server is available (if using -p/--proxy)")
                print(f"4. Supported platforms: {', '.join(PlatformConfig.get_supported_platforms())}")
...
async def main():
        try:
                ...
                formatted = OutputFormatter.format_response(
